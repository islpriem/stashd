...
def test_dev_cluster_template_is_valid(tmp_path: Path) -> None:
    ...
    rendered.write_text(template.replace("@ROOT@", str(tmp_path)))
    ...
    assert config.storage("LOC2HOT").root == tmp_path / "dev/storage/loc2hot"


def test_dev_storage_roots_stay_inside_the_repository(tmp_path: Path) -> None:
    rendered = tmp_path / "cluster.yaml"
    rendered.write_text((DEV / "cluster.yaml.in").read_text().replace("@ROOT@", str(tmp_path)))

    for storage in load_cluster_config(rendered).storages:
        assert tmp_path in storage.root.parents


@pytest.mark.parametrize("name", ["controller.yaml", "daemon-hot1.yaml", "cluster.yaml.in"])
